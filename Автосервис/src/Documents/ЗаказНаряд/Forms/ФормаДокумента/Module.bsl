...
//		Сообщить("Имя варианта (строка): " + ИмяВарианта);
    ...
    Если ЗначениеЗаполнено(ДокументОбъект.Калькуляция) Тогда
        ...
        Для Каждого СтрокаРаботы Из КалькуляцияОбъект.Работы Цикл
            ...
            НоваяСтрока.НормаЧас = СтрокаРаботы.НормаЧас;
            НоваяСтрока.Сумма = СтрокаРаботы.Количество*СтрокаРаботы.НормаЧас*ЦенаНормаЧаса;
        КонецЦикла;
        Для Каждого СтрокаДетали Из КалькуляцияОбъект.Автодетали Цикл
        	...
        	Если НомерВарианта = 1 Тогда
        		НоваяСтрока.Материал = СтрокаДетали.Материал1;
        		НоваяСтрока.Цена = СтрокаДетали.Цена1;
        		НоваяСтрока.Сумма = СтрокаДетали.Цена1;
        	ИначеЕсли НомерВарианта = 2 Тогда
        		НоваяСтрока.Материал = СтрокаДетали.Материал2;
        		НоваяСтрока.Цена = СтрокаДетали.Цена2;
        		НоваяСтрока.Сумма = СтрокаДетали.Цена2;
        	Иначе 
        		НоваяСтрока.Материал = СтрокаДетали.Материал3;
        		НоваяСтрока.Цена = СтрокаДетали.Цена3;
        		НоваяСтрока.Сумма = СтрокаДетали.Цена3;
        	КонецЕсли;   

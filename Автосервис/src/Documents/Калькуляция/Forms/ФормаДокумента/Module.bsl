...
Процедура ПечатьЧекаНаСервере(ТабДок)
	...
	Для Каждого СтрокаРаботы Из Объект.Работы Цикл
    	...
    	Таблица.Параметры.Сумма = СтрокаРаботы.НормаЧас * СтрокаРаботы.Количество * СтоимостьНормаЧаса.Стоимость;
    	СуммаПоРаботам = СуммаПоРаботам + СтрокаРаботы.НормаЧас * СтрокаРаботы.Количество * СтоимостьНормаЧаса.Стоимость;
	...
	Для Каждого СтрокаДеталей Из Объект.Автодетали Цикл
    	...
    	ИтогСумма2 = ИтогСумма2 + СтрокаДеталей.Сумма2;
    	ИтогСумма3 = ИтогСумма3 + СтрокаДеталей.Сумма3;    	
